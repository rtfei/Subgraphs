...
import {
  ...
  ProposalExecuted,
  VoteCast
} from "../generated/GovernorAlpha2/GovernorAlpha2";
...
export function handleProposalCreated(event: ProposalCreated): void {
  let proposal = fetchProposal(event.params.id.toString() + " - contract - " + event.address.toHexString());
...
export function handleVoteCast(event: VoteCast): void {
  let proposal =  fetchProposal(event.params.proposalId.toString() + " - contract - " + event.address.toHexString());
  let governance = getGovernanceEntity();
  let voteId = event.params.voter
    .toHexString()
    .concat("-")
    .concat(event.params.proposalId.toString() + " - contract - " + event.address.toHexString());
