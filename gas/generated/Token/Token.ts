...
export class Token extends ethereum.SmartContract {
  ...
  vestStarted(): boolean {
    let result = super.call("vestStarted", "vestStarted():(bool)", []);

    return result[0].toBoolean();
  }

  try_vestStarted(): ethereum.CallResult<boolean> {
    let result = super.tryCall("vestStarted", "vestStarted():(bool)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
...
export class StartVestCall extends ethereum.Call {
  get inputs(): StartVestCall__Inputs {
    return new StartVestCall__Inputs(this);
  }

  get outputs(): StartVestCall__Outputs {
    return new StartVestCall__Outputs(this);
  }
}

export class StartVestCall__Inputs {
  _call: StartVestCall;

  constructor(call: StartVestCall) {
    this._call = call;
  }

  get tokenLockAddress(): Address {
    return this._call.inputValues[0].value.toAddress();
  }
}

export class StartVestCall__Outputs {
  _call: StartVestCall;
