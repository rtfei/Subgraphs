...
export class Governor extends ethereum.SmartContract {
  ...
  try_initialProposalId(): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "initialProposalId",
      "initialProposalId():(uint256)",
      []
  ...
  rgt(): Address {
    let result = super.call("rgt", "rgt():(address)", []);

    return result[0].toAddress();
  }

  try_rgt(): ethereum.CallResult<Address> {
    let result = super.tryCall("rgt", "rgt():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
